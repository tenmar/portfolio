...
function constructStates(amount: any, horizontal: boolean = true, duration: number = BASE_DURATION): AnimStyles {
  const type = horizontal ? 'X' : 'Y'
  const trans = `transform ${duration}ms, opacity ${duration}ms ease-in-out`
  return {
    entering: { transform: `translate${type}(0)`, opacity: 1, transition: trans },
    entered: { transform: `translate${type}(0)`, opacity: 1, transition: trans },
    exiting: { transform: `translate${type}(${amount})`, opacity: 0, transition: trans },
    exited: { transform: `translate${type}(${amount})`, opacity: 0, transition: trans },
...
function getBGClassStyle(from: CameraPos, to: CameraPos, duration: number = BASE_DURATION): AnimStyles {
...
class Camera extends React.Component<ICameraProps, ICameraState> {
  ...
  getControlStyle(transitionState: TransitionStatus, duration: number = BASE_DURATION / 2): CSSProperties {
    const { currentPosition, lastPosition } = this.state
  ...
  renderReturnControl(transitionState: TransitionStatus) {
    ...
    let handleOnClick: React.MouseEventHandler<HTMLDivElement> = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    return <div style={{ ...this.getControlStyle(transitionState) }}>
  ...
  renderDecoration(state: TransitionStatus, duration: number = BASE_DURATION / 2) {
    const { currentPosition, lastPosition } = this.state


    let isHorizontal = this.checkPos(CameraPos.LEFT) || this.checkPos(CameraPos.RIGHT)
    let isVertical = this.checkPos(CameraPos.UP) || this.checkPos(CameraPos.DOWN)
    ...
    const enterTrans = `all ${duration}ms ease-out ${duration + duration/2}ms`
    ...
    return <div className="foreground-decoration"
      style={{
        position: 'fixed',
        zIndex: 10, display: 'flex', flexDirection: isHorizontal ? 'column' : isVertical ? 'row' : 'column',
        ...style,
        ...transitionStyle[state]
      }}>
      <div style={{ width: 400, height: 400, transform: 'translateX(200px) translateY(-200px)', borderRadius: 200, backgroundColor: 'black' }} />
      <div style={{ flex: 1 }} />
      <div style={{ flex: 1 }} />
      <div style={{ width: 350, height: 350, borderRadius: 175,transform: 'translateX(175px) translateY(175px)', backgroundColor: 'black' }} />
    </div>
  }
